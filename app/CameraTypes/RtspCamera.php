...
abstract class RtspCamera extends CameraType
{
    ...
    public function startRecording(Camera $camera, Recording $recording)
    {
        ...
        // $processId = FFMpegCommand::run($this->getRtspUrl($camera), $outputFile, '-tag:v hvc1 -master_pl_name master.m3u8 -f hls -hls_time ' . config('taggy-recorder.video-conversion.segment-duration') . ' -hls_segment_type fmp4 -hls_list_size 0 -hls_segment_filename ' . $outputDirectory . '/video-%05d.m4s -c copy');
        $processId = FFMpegCommand::run($this->getRtspUrl($camera), $outputFile, '-tag:v hvc1 -master_pl_name master.m3u8 -f hls -hls_time ' . config('taggy-recorder.video-conversion.segment-duration') . ' -hls_list_size 0 -hls_segment_filename ' . $outputDirectory . '/video-%05d.m4s -c copy');
