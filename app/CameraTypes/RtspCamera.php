...
abstract class RtspCamera extends CameraType
{
    public function startRecording(Camera $camera)
    {
        ...
        $processId = $this->runFFmpegCommand($this->getRtspUrl($camera), $outputFile, '-c:v copy -c:a copy -f segment -segment_list m3u8 -segment_list_type m3u8 -segment_time ' . config('taggy-recorder.video-conversion.segment-duration'));
